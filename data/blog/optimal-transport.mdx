...
$$EMD(P_r, P_\theta) \approxeq sup \mathbb{E}_{p_r}$$
1-Lipschitz condition ensures there is smoothness between two distributions on the discriminator corresponding to $\mathbb{E}_{p_r}[f(x)]$
...
The **main difference** between Wasserstein GAN and regular GAN is that it uses linear activation instead of sigmoid in discriminator and reduces the logs in
